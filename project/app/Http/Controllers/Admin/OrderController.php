...
class OrderController extends Controller
{
    ...
    //*** JSON Request
    public function datatables($status,Request $request)
    {
        // dd($request->vendorId);
        if($request->vendorId){
            $datas = Order::orderBy('id','desc')->where('user_id','=',$request->vendorId)->get(); 
        }else{
            if($status == 'pending'){
    ...
    {
        // dd($request-$request->vendorId);
        return view('admin.order.index')->with('request',$request->vendorId);
    }
