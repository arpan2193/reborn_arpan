	    ...
	    public function datatables()
	    {
	        $datas = User::where('is_vendor','=',2)->orWhere('is_vendor','=',1)->orderBy('id','desc')->get();
	         ...
	         return Datatables::of($datas)
                ...
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 2]).'" '.$s.'>Unblock</option>'.
                '<option value="'. route('admin-vendor-st',['id1' => $data->id, 'id2' => 1]).'" '.$ns.'>Block</option></select></div>';
                                }) 
	                            ->addColumn('action', function(User $data) {
                                    ...
                                    <a href="' . route('admin-vendor-order',$data->id) . '" class="verify"> <i class="fas fa-luggage-cart"></i> Orders</a>
                                    <a href="' . route('admin-vendor-show',$data->id) . '" > <i class="fas fa-eye"></i> View</a><a href="' . route('admin-vendor-edit',$data->id) . '"> <i class="fas fa-edit"></i> Edit</a><a href="javascript:;" data-href="' . route('admin-vendor-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i> Delete</a></div></div>';
	                            }) 
