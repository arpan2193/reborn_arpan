...
class ProductController extends Controller
{
     ...
     public function datatables()
     {
          ...
          return Datatables::of($datas)
                             ->editColumn('name', function(Product $data) {
                                 ...
                                 $id = '<small>'.__("ID").': <a href="'.route('front.productdetails', $data->slug).'" target="_blank">'.sprintf("%'.08d",$data->id).'</a></small>';
 
                                 $id3 = $data->type == 'Physical' ?'<small class="ml-2"> '.__("SKU").': <a href="'.route('front.productdetails', $data->slug).'" target="_blank">'.$data->sku.'</a>' : '';
    ...
    // {
    //     // dd($request->id);
    //     return view('admin.product.index')->with('request', $request);
    // }
